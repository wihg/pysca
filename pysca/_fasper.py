...
# Note: This module is based on the code available at
#
#  http://www.astropython.org/snippet/2010/9/Fast-Lomb-Scargle-algorithm
#
# with some modifications which allow the explicit selection of an upper
# frequency (hifreq) instead of just a factor (hifac).
#
...
# W.H: Adapted the fasper function to take a frequency limit 'hifreq', up to
# which the periodogram is calculated. Can be used instead of the parameter
# 'hifac'.
      
def fasper(x,y,ofac, hifac=None, hifreq=None, MACC=4):
  """ function fasper
  ...
  assert hifac == None or hifreq == None
  if hifac == None and hifreq == None:
      hifac = 1.0
  ...
  if hifac != None:
    nout = ceil(0.5*ofac*hifac*n)
  elif hifreq != None:
    nout = ceil(hifreq/df)
  nout = long(nout)

  nfreqt = long(2.0*nout*MACC)   #Size the FFT as next power
  ...
  #W.H: return variance instead of fap/prob
  return wk1,wk2,nout,jmax,var
