...
def prewhiten(data, freqs, startpars=None):
    ...
    if startpars == None:
        optpar, cov, infodict, mesg, ier = leastsq(minfunc, r_[ones(freqs.size), 0.5*ones(freqs.size)], args=(data[:,0], data[:,1], freqs), full_output=1, Dfun= dfunc, col_deriv=1)
    else:
        ...
        optpar, cov, infodict, mesg, ier = leastsq(minfunc, startpars.T.flatten(), args=(data[:,0], data[:,1], freqs), full_output=1, Dfun= dfunc, col_deriv=1)
    ...
    # Calculate residuals
    fitcurve = harmfunc(data[:,0], normoptpar, freqs)
    residu = data[:,1] - fitcurve
    residat = c_[data[:,0], residu]

    return residat, c_[abs(optam), normphase]